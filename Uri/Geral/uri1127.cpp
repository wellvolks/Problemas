...
char Mu[300030], Tre[300030];

int mapa[] = { 0, 2, 3, 5, 7, 8, 10 };
...
bool stringMatching(int m, int N){
	int lo = 0, hi = N-1, mid = lo;
	while( lo < hi ){
		mid = (lo + hi) >> 1;
		int res = memcmp( Mu + SA[mid], Tre, m );
		if( res >= 0 ) hi = mid;
		else lo = mid + 1;
	}
	if( memcmp( Mu + SA[lo], Tre, m) ) return false;
	return true;
}
...
int calcDist( int a, int b ){
	int i = max(a,b);
	int j = min(a,b);
	return max(i-j, 12 - (i-j));
}

int main(){
	ios::sync_with_stdio(false);
	int n, m;
	string  str, musica, trecho;
	while( cin >> n >> m && (n + m) ){
		cin >> musica;
		int pm = 0, pt = 0;
		REP(i,n-1){
			int distI = mapa[(int)(musica[0] - 'A')];
			if( musica.size() > 1 ){
				if( musica[1] == '#' ) distI++;
				else distI--;
			}
			cin >> musica;
			int distJ = mapa[musica[0]- 'A'];
			if( musica.size() > 1 ){
				if( musica[1] == '#' ) distJ++;
				else distJ--;
			}
			int dist = calcDist(distI, distJ);
			Mu[pm++] = char((dist/10) + '0');
			Mu[pm++] = char((dist%10) + '0');
			Mu[pm++] = ' ';
		}
		cin >> trecho;
		REP(i,m-1){
			int distI = mapa[trecho[0] - 'A'];
			if( trecho.size() > 1 ){
				if( trecho[1] == '#' ) distI++;
				else distI--;
			}
			cin >> trecho;
			int distJ = mapa[trecho[0]- 'A'];
			if( trecho.size() > 1 ){
				if( trecho[1] == '#' ) distJ++;
				else distJ--;
			}
			int dist = calcDist(distI, distJ);
			Tre[pt++] = char((dist/10) + '0');
			Tre[pt++] = char((dist%10) + '0');
			Tre[pt++] = ' ';
		}
		Mu[pm] = '.';
		Tre[pt] = '.';
		SA_IS( Mu, SA, pm, 150, 83 ); 
		cout << ((stringMatching(pt, pm)) ? ("S") : ("N")) << "\n";
	}
	return 0;
}